...
export class Rule extends Lint.Rules.TypedRule {
  public static FAILURE_STRING =
    "yield result should be typed if result is used: (__EXPRESSION__) as 'ResultType'";
...
class StrictYieldTypeWalker extends Lint.AbstractWalker<Set<string>> {
  ...
  private checkYieldExpression(node: ts.Node) {
    ...
    if (
      isParentNodeExist &&
      this.checkParentType(node) &&
      !this.validateAsExpression(node.parent.parent, node)
    ) {
      this.addFailureAtNode(
        node,
        Rule.FAILURE_STRING.replace("__EXPRESSION__", node.getText())
      );
    }
  }
  ...
  private validateAsExpression(
    ...
    if (result) {
      ...
      let yieldType =
        yieldExpression.getChildCount() > 0 &&
        this.checker.getTypeAtLocation(yieldExpression.getChildAt(1));
      let yieldPromiseType =
        this.checker["getPromisedTypeOfPromise"](yieldType);

      if (yieldPromiseType !== castType) {
        const error = !!yieldPromiseType
          ? "yield return type '" +
            this.checker.typeToString(yieldPromiseType) +
            "' is not equal to " +
            "casting type '" +
            this.checker.typeToString(castType) +
            "'"
          : "Yield expression does't return Promise type: " +
